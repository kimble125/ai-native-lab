# GPT 프롬프팅 가이드 (주입 단위) — guide-gpt.md

> **세션 ①번에 붙여넣는 파일.** 출처: https://developers.openai.com/api/docs/guides/prompt-guidance
> (+ 필요시 `.../guides/prompt-engineering`, `.../guides/reasoning-best-practices`)
> 자동 추출이 안 되는 페이지라, **아래 [공식 본문] 칸에 브라우저에서 직접 복사해 채워 넣으세요.**
> 프롬프트 원칙 섹션만 — API/SDK/요금/함수호출 챕터는 제외(컨텍스트 절약).
...
> 아래는 널리 알려진 안정적 원칙입니다. **최신(GPT-5.5) 신기능·수치는 반드시 위 [공식 본문]으로 대체/검증**하세요.

- **명확·구체적 지시**: 원하는 것을 모호함 없이. "좋게 써줘" ✕ → "9개 고정 섹션, 각 3문장, 존댓말" ◯
- **역할·맥락 부여**: system/developer 메시지에 페르소나·목표·제약을 명시.
- **출력 형식 고정**: 원하는 구조를 예시로 보여주기(few-shot) + 형식 명시(JSON/표/마크다운).
- **지시는 앞쪽에, 구분자 사용**: 긴 입력은 `###`나 따옴표로 데이터/지시를 분리.
- **추론(reasoning) 모델에선 '덜' 지시**: 사고과정을 일일이 시키지 말고 목표·제약만 주고 추론은 모델에 맡김. reasoning effort로 깊이 조절.
- **에이전트 적극성 제어**: 너무 많이/적게 행동하지 않도록 범위·중단 조건을 명시.
